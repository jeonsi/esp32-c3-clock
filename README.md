...
- 좌하단 8×8 **시간 소스 아이콘**(BLE 룬 / Wi-Fi 아크, CYD의 링크 아이콘과 같은 위치)은 선택된 동기화 방식을 나타내며, 링크가 끊겨 있는 동안 1초 간격으로 깜박입니다. 아날로그 화면에도 같은 위치에 표시됩니다. BLE 듀티사이클로 라디오가 의도적으로 꺼져 있는 대기 시간에는 고정 표시되고, 재동기화 창이 열려 폰을 기다리는 동안에만 깜박입니다. **재동기화 창이 동기화 없이 통째로 지나가면(1분) 즉시, 또는 마지막 성공 동기화가 `SYNC_STALE_MS`(주기+5분)를 넘으면 아이콘이 반전 박스로 바뀝니다** — 표시 중인 시간이 보정 없이 흘러가고 있다는 경고입니다.
...
- Wi-Fi 모드는 AP 비컨을 계속 수신해야 해서 슬립하지 않습니다(기존과 동일).
