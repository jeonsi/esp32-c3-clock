...
- **좌상단에 배터리 아이콘**(채움 = 잔량%)이 표시됩니다 — 날짜 줄이 중앙정렬이라 좌상단은 비어 있고, 우상단은 `SLEEP_DEBUG` 수치가 쓰므로 겹치지 않습니다. 잔량 10%(`VBAT_LOW_PCT`) 이하면 초마다 깜빡입니다. 전압→%는 리튬이온 OCV 곡선의 구간 보간(4.2V=100 … 3.5V≈8 … 3.0V=0)입니다.
