...
- 좌하단 8×8 **시간 소스 아이콘**(BLE 룬 / Wi-Fi 아크, CYD의 링크 아이콘과 같은 위치)은 선택된 동기화 방식을 나타내며, 링크가 끊겨 있는 동안 1초 간격으로 깜박입니다. 아날로그 화면에도 같은 위치에 표시됩니다. BLE 듀티사이클로 라디오가 의도적으로 꺼져 있는 대기 시간에는 고정 표시되고, 재동기화 창이 열려 폰을 기다리는 동안에만 깜박입니다. **마지막 성공 동기화가 2주기(`SYNC_STALE_MS`, 기본 2시간)를 넘으면 아이콘이 반전 박스로 바뀝니다** — 표시 중인 시간이 보정 없이 흘러가고 있다는 경고입니다.
...
- **라디오 듀티사이클**(`BLE_DUTY_CYCLE`, 기본 켜짐) — 동기화가 끝나면 BLE 스택을 내려 라디오를 끄고, 1시간 뒤 다시 켜서 광고를 시작합니다. 본딩 키는 NVS에 남아 있어 **아이폰이 광고를 보고 스스로 재연결**합니다(보통 수 초). **동기화와 ANCS 구독이 모두 확인되면 2초(`BLE_SETTLE_MS`) 뒤 바로 라디오를 꺼서** 불안정한 링크에서 연결이 끊겼다 붙었다 반복되는 것을 피하고, 구독이 아직 안 된 경우(첫 페어링의 "알림 공유" 대기 등)에만 동기화 후 최대 60초(`BLE_LINGER_MS`)까지 켜둡니다. 폰이 없어 동기화가 안 되는 창은 3분(`BLE_SYNC_TIMEOUT_MS`) 뒤 닫고 다음 주기에 재시도합니다. 실측 평균 소비전류(80MHz): Wi-Fi 모드 ~25mA, BLE 상시 켜짐 ~50mA, **BLE 듀티사이클 ~20mA**. `BLE_DUTY_CYCLE 0`이면 CYD처럼 상시 연결 유지
