...
| 부저 (선택) | 수동 피에조의 한쪽을 **GPIO10**(`SPK_PIN`)에, 반대쪽은 둘 중 하나로: ① **진짜 GND** (`SPK_GND_PIN -1`), ② SuperMini처럼 GND 핀이 부족하면 **GPIO20 가상 GND** (`SPK_GND_PIN 20`, LOW 고정 — 나란한 10–20 두 핀에 바로 꽂힘). 피에조는 전류를 거의 먹지 않아 어느 쪽이든 GPIO 직결로 충분. 능동 부저(자체 발진)는 전류를 많이 먹어 트랜지스터 구동 필요. 부저 자체가 없으면 `SPK_PIN -1` |
...
| `SPK_GND_PIN` | 20 | 피에조 **반대쪽 다리**의 위치(`SPK_PIN`과 별개): 20 = GPIO20을 LOW로 고정한 가상 GND(GPIO10 바로 옆), **-1 = 진짜 GND에 배선한 경우**(부저는 그대로 사용됨) |
