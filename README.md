...
- **라디오 듀티사이클**(`BLE_DUTY_CYCLE`, 기본 켜짐) — 동기화가 끝나면 BLE 스택을 내려 라디오를 끄고, 1시간 뒤 다시 켜서 광고를 시작합니다. 본딩 키는 NVS에 남아 있어 **아이폰이 광고를 보고 스스로 재연결**합니다(보통 수 초). 첫 페어링의 "알림 공유" 단계를 위해 동기화 후 60초(`BLE_LINGER_MS`)는 켜두고, 폰이 없어 동기화가 안 되는 창은 3분(`BLE_SYNC_TIMEOUT_MS`) 뒤 닫고 다음 주기에 재시도합니다. 실측 평균 소비전류(80MHz): Wi-Fi 모드 ~25mA, BLE 상시 켜짐 ~50mA, **BLE 듀티사이클 ~20mA**. `BLE_DUTY_CYCLE 0`이면 CYD처럼 상시 연결 유지
...
| `BLE_LINGER_MS` | 60초 | 동기화 후 라디오를 더 켜두는 시간(첫 페어링의 ANCS 허용 단계용) |
