...
- 좌상단 8×8 **시간 소스 아이콘**(BLE 룬 / Wi-Fi 아크)은 선택된 동기화 방식을 나타내며, 링크가 끊겨 있는 동안 1초 간격으로 깜박입니다. 아날로그 화면에도 같은 위치에 표시됩니다. BLE 듀티사이클로 라디오가 의도적으로 꺼져 있는 대기 시간에는 고정 표시되고, 재동기화 창이 열려 폰을 기다리는 동안에만 깜박입니다.
