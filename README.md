...
### 절전 (BLE 모드)
- `SLEEP_ENABLE 1`(기본)이면 **BLE 모드에서 라디오가 꺼져 있는 동안 초 갱신 사이를 light sleep**으로 보냅니다(약 97% 수면) — 평균 소비전류 ~20mA → **~6mA**(OLED 자체 소비가 하한). 시간·타이머는 슬립을 가로질러 정상 진행되어 듀티사이클·시보·야간 모드에 영향이 없습니다.
- 버튼은 GPIO 웨이크로 누르는 순간 깨어나 제스처가 그대로 동작하고, 버튼 조작 후 1.5초간은 슬립하지 않아 멀티클릭 타이밍이 유지됩니다.
- **시리얼 모니터가 열려 있으면(DTR) 슬립하지 않습니다** — 슬립이 USB CDC 세션을 끊기 때문에, 디버깅 중에는 자동으로 항상 깨어 있습니다.
- Wi-Fi 모드는 AP 비컨을 계속 수신해야 해서 슬립하지 않습니다(기존과 동일).
- ⚠️ 보조배터리 급전 시: 6mA는 대부분의 보조배터리 자동 꺼짐 문턱보다 낮으므로, **저전류(trickle) 모드가 있는 팩**이 필요할 수 있습니다.
...
| `SLEEP_ENABLE` | 1 | BLE 모드에서 초 갱신 사이 light sleep(~6mA). 0이면 항상 깨어 있음 |
