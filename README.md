...
- 부저를 사용하면(`SPK_PIN` ≥ 0) 시간 소스 아이콘 옆에 **사운드 상태 아이콘**이 표시됩니다 — 소리 켜짐은 음파가 있는 스피커, 뮤트는 빗금 친 스피커(CYD와 동일한 표현). 상태는 NVS에 저장되며 `OVERRIDE_SOUND`(0=뮤트, 1=켬)로 소스에서 변경합니다.
...
- **뮤트**: 모든 소리는 `sound_on` 상태(NVS `snd`)로 게이트되며, 좌하단 스피커 아이콘이 상태를 표시. 변경은 `OVERRIDE_SOUND`로(터치·여분 제스처가 없어 소스 설정 방식)
