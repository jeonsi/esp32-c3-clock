...
```
   금 9-10                  ← 요일(한글 1글자, 굴림 12px) + 월-일(DSEG7 11px, 연도·앞자리 0 없음). 빨간날은 요일 반전
 P                          ← PM 마커 (DSEG14 11px, 시간 좌측 상단). AM은 표시 없음(자리는 유지되어 블록이 안 움직임)
   11:58         42         ← HH:MM (DSEG7 Bold 28px) + 초 (DSEG7 18px, 분 높이의 ~2/3, 하단 정렬)
 ᛒ🔊 - 8.15 추분  추석        ← 시간 소스·사운드 아이콘 + 음력 날짜·현재 절기(절입 당일 반전) + [공휴일(반전) | 세시명절]
```

- 세 줄 모두 **중앙 정렬**. 시간 블록은 1행과 3행 사이 36px 밴드의 정중앙에 놓입니다.
- 12시간제에서 시의 앞자리는 `1` 아니면 빈칸이므로, CYD처럼 `1`의 잉크 폭만 예약해 한 자리 시(`9:05`)일 때 왼쪽에 죽은 공간이 생기지 않습니다. 24시간제는 `09:05`처럼 두 자리로 표시하고 A/P 마커 없이 초와의 간격을 0으로 줄여 128px에 맞춥니다.
- 하단 줄은 아이콘 바로 옆부터 왼쪽 정렬로 **음력 날짜 → 절기 → 기념일** 순서로 붙여서 배치합니다(음력–절기는 좁은 간격) — 밀착 배치로 남는 공간이 기념일 이름 몫입니다. 음력 표식은 `-`(윤달은 `- 윤`), 숫자는 DSEG. 공휴일 이름은 반전, 정월대보름·단오·칠석 같은 세시명절은 일반 글자. 한 줄에 다 들어가지 않으면 절기 → 음력 순으로 생략하고 이름은 항상 남깁니다.
- 색이 없는 흑백 패널이라 CYD의 빨간 요일·초록 절기는 **반전 박스**로 대신합니다.
- 부저를 사용하면(`SPK_PIN` ≥ 0) 시간 소스 아이콘 옆에 **사운드 상태 아이콘**이 표시됩니다 — 소리 켜짐은 음파가 있는 스피커, 뮤트는 빗금 친 스피커(CYD와 동일한 표현). BOOT 버튼 **트리플 클릭**으로 토글하며 NVS에 저장됩니다.
- 좌하단 8×8 **시간 소스 아이콘**(BLE 룬 / Wi-Fi 아크, CYD의 링크 아이콘과 같은 위치)은 선택된 동기화 방식을 나타내며, 링크가 끊겨 있는 동안 1초 간격으로 깜박입니다. 아날로그 화면에도 같은 위치에 표시됩니다. 듀티사이클로 라디오가 의도적으로 꺼져 있는 대기 시간(BLE·Wi-Fi 공통)에는 고정 표시되고, 재동기화 창이 열려 상대를 기다리는 동안에만 깜박입니다. **재동기화 창이 동기화 없이 통째로 지나가면(1분) 즉시, 또는 마지막 성공 동기화가 `SYNC_STALE_MS`(주기+5분)를 넘으면 아이콘이 반전 박스로 바뀝니다** — 표시 중인 시간이 보정 없이 흘러가고 있다는 경고입니다.
...
- **여러 AP 등록 가능**: `secrets.h`에 `WIFI_APS` 목록을 정의하면 스캔해서 **실제로 보이는 AP 중 신호가 가장 센 곳**에 붙습니다(비동기 스캔이라 초침이 멈추지 않음). 접속이 15초간 안 되는 AP는 그 라운드에서 제외하고 다음으로 센 AP로 넘어가며, 목록의 AP가 하나도 안 보이면 10초(`WIFI_RETRY_MS`)마다 재스캔. 기존처럼 `WIFI_SSID`/`WIFI_PASSWORD` 하나만 둬도 동작합니다
...
- **라디오 듀티사이클**(`BLE_DUTY_CYCLE`, 기본 켜짐) — 동기화가 끝나면 BLE 스택을 내려 라디오를 끄고, 1시간 뒤 다시 켜서 광고를 시작합니다. 본딩 키는 NVS에 남아 있어 **아이폰이 광고를 보고 스스로 재연결**합니다(보통 수 초). **동기화와 ANCS 구독이 모두 확인되면 2초(`BLE_SETTLE_MS`) 뒤 바로 라디오를 꺼서** 불안정한 링크에서 연결이 끊겼다 붙었다 반복되는 것을 피하고, 구독이 아직 안 된 경우(첫 페어링의 "알림 공유" 대기 등)에만 동기화 후 최대 60초(`BLE_LINGER_MS`)까지 켜둡니다. 폰이 없어 동기화가 안 되는 창은 1분(`BLE_SYNC_TIMEOUT_MS`) 뒤 닫고(이때 소스 아이콘이 즉시 반전) 다음 주기에 재시도합니다. 실측 평균 소비전류(80MHz): Wi-Fi 모드 ~25mA, BLE 상시 켜짐 ~50mA, **BLE 듀티사이클 ~20mA**. `BLE_DUTY_CYCLE 0`이면 CYD처럼 상시 연결 유지
...
공통
- 비차단 부팅: Wi-Fi 연결(또는 BLE 페어링 안내)/시간 동기화 진행 상황을 화면에 표시하고, 실패해도 무한 재시도. 대기 중에도 버튼은 동작합니다(3초 길게 눌러 소스 전환 가능)
- **소프트 리부팅(소스 전환 등)은 시스템 시각이 살아남으므로** 연결을 기다리지 않고 즉시 시계를 표시하고, 첫 (재)동기화는 듀티사이클에 맡깁니다 — AP/폰이 없으면 1분 뒤 창이 닫히며 소스 아이콘이 반전됩니다
- 화면은 초가 바뀌는 순간(10ms 폴링)에만 다시 그림. I2C는 400kHz로 구동해 프레임 전송(~23ms) 동안 버튼을 못 읽는 공백을 최소화

### BOOT 버튼
| 조작 | 동작 | 배너 |
|---|---|---|
| 짧게 1회 | 디지털 ↔ 아날로그 | — |
| 짧게 2회 (0.7초 내) | 12시간제 ↔ 24시간제 | `12시간제` / `24시간제` |
| 짧게 3회 | 소리 뮤트 ↔ 켬 (부저 사용 시) | `소리 켜짐` / `소리 꺼짐` |
| 1~3초 | 야간 모드 ON/OFF | `야간 모드 켜짐` / `야간 모드 꺼짐` |
| 3초 이상 | 시간 소스 전환 후 재부팅 | `BLE 모드로 재시작` / `Wi-Fi 모드로 재시작` |

- 다섯 설정 모두 NVS(`Preferences`)에 저장되어 재부팅 후 유지. 설정이 바뀔 때만 기록
- 클릭은 마지막 클릭 후 0.7초(`DOUBLE_CLICK_MS`) 동안 추가 클릭이 없을 때 반영됩니다(1회=화면, 2회=12/24h, 3회=뮤트) — 클릭 사이 간격도 0.7초까지 인정됩니다. 뮤트를 해제하면 확인용 비프가 한 번 울립니다
...
### 절전 (BLE 모드)
- `SLEEP_ENABLE 1`(기본)이면 **BLE 모드에서 라디오가 꺼져 있는 동안 초 갱신 사이를 light sleep**으로 보냅니다(실측 약 96% 수면) — 평균 소비전류 실측 **~25mA → ~10mA**. 시간·타이머는 슬립을 가로질러 정상 진행되어 듀티사이클·시보·야간 모드에 영향이 없습니다.
- 버튼(GPIO9, SCL 공유)은 GPIO 웨이크로 누르는 순간 깨어나 제스처가 그대로 동작하고, 버튼 조작 후 1.5초간은 슬립하지 않아 멀티클릭 타이밍이 유지됩니다. 슬립 중에는 이 핀에 **슬립 전용 패드 설정(입력+풀업, 출력 해제)** 을 적용합니다 — 활성 설정을 유지하면 클럭이 멈춘 I2C 컨트롤러 출력을 따라 SCL이 LOW로 끌려 즉시 깨어나 버립니다.
- **USB 호스트(PC)가 붙어 있으면 슬립하지 않습니다** — 슬립이 USB CDC 세션을 끊기 때문. 호스트 판별은 `if (Serial)`이 아니라 **SOF 패킷 감지**로 합니다: HWCDC의 bool은 보조배터리(VBUS만 존재)에서도 참이 되어 슬립을 영원히 막지만, SOF는 진짜 호스트만 1ms마다 보냅니다.
- Wi-Fi 모드도 `WIFI_DUTY_CYCLE 1`(기본)이면 라디오가 꺼져 있는 사이 똑같이 슬립합니다. `WIFI_DUTY_CYCLE 0`(상시 연결)일 때만 AP 비컨 수신 때문에 슬립하지 않습니다.
- `SLEEP_DEBUG 1`로 빌드하면 우상단에 직전 1초의 수면 비율(정상 ~96)이 표시되고, 문제 시 원인 문자가 앞에 붙습니다(`s`=USB 호스트 감지, `g`=핀 LOW 즉시 기상, `.`=슬립 거부, `-`=가짜 GPIO 기상, `w`/`b`/`i`/`k`=라디오 켜짐/버튼/버튼 대기창/백오프).
- ⚠️ 보조배터리 급전 시: 부하가 낮으면 자동 꺼짐 문턱에 걸릴 수 있으므로, 필요하면 **저전류(trickle) 모드가 있는 팩**을 사용하세요.

### 배터리 잔량 표시 + 저전압 보호
- 저항 2개만 배선하면 됩니다: **배터리+(팩이 USB로 급전하면 5V 핀) → 100kΩ → GPIO3(`VBAT_ADC_PIN`) → 100kΩ → GPIO0(`VBAT_GND_PIN`)**. 분압으로 전압을 절반(4.2V→2.1V)으로 접어 ADC 범위에 넣고, 16회 평균 + EMA로 노이즈를 잡으므로 콘덴서는 없어도 됩니다.
- 분압기 바닥은 부저(`SPK_GND_PIN`)처럼 **LOW로 박은 GPIO를 가상 GND**로 씁니다 — 흐르는 전류가 21µA뿐이라 진짜 GND와의 차이가 1mV 미만이고, 하나뿐인 GND 핀은 OLED가 그대로 씁니다. 슬립 중에도 핀이 LOW를 유지하도록 슬립용 패드 설정(출력+풀다운)을 지정합니다. 진짜 GND에 배선했다면 `VBAT_GND_PIN -1`.
- **좌상단에 배터리 아이콘**(채움 = 잔량%)과 잔량 % 숫자가 표시됩니다 — 날짜 줄이 중앙정렬이라 좌상단은 비어 있고, 우상단은 `SLEEP_DEBUG` 수치가 쓰므로 겹치지 않습니다. 잔량 10%(`VBAT_LOW_PCT`) 이하면 초마다 깜빡입니다. 전압→%는 리튬이온 OCV 곡선의 구간 보간(4.2V=100 … 3.55V≈10 … 3.3V=0 — 0%는 셀 한계가 아니라 아래 보호 지점 기준)입니다.
- **저전압 보호**: EMA 전압이 3.3V(`VBAT_SHUTDOWN_MV`) 미만으로 3회 연속(30초) 측정되면 OLED를 끄고 deep sleep(~50µA)으로 들어가 셀을 과방전 구간에서 지킵니다. 1시간마다 깨어 전압을 재확인하므로 **충전기에 꽂아두면 스스로 복귀**합니다(리셋 버튼도 가능). 시각은 재부팅 후 BLE로 다시 받아오고 설정은 NVS에 남습니다.
- 배선 전에 플래시해도 안전합니다: 핀이 떠 있어 리튬이온답지 않은 값(2.5V 미만/4.5V 초과)이 읽히면 아이콘과 보호가 모두 비활성화됩니다. 배선 후에는 `BATTERY_DEBUG 1`로 빌드하면 아이콘 대신 측정 전압(mV)이 표시되니, 멀티미터 실측과 비교해 다르면 `VBAT_DIV`를 실측비로 보정하세요. `VBAT_ADC_PIN -1`이면 기능 전체가 빠집니다.
...
### 정시 시보
- 매 정시에 카시오 시계처럼 짧은 비프음 2회("삐삑", 60ms×2, 2.5kHz). 기본은 주간(7시~22시)에만 울리고 밤에는 조용함 (`CHIME_FROM/TO_HOUR`로 조정, `HOURLY_CHIME 0`으로 끔)
- 부팅 시 같은 소리를 한 번 울려 배선을 확인 (`BOOT_BEEP 0`으로 끔)
- **뮤트**: 모든 소리는 `sound_on` 상태(NVS `snd`)로 게이트되며, 좌하단 스피커 아이콘이 상태를 표시. **BOOT 버튼 트리플 클릭**으로 토글(해제 시 확인 비프), `OVERRIDE_SOUND`로 소스에서 강제도 가능
- 수동 피에조 기준이며(`ledcWriteTone`으로 구동), 소리가 작으면 `CHIME_TONE_HZ`를 부저의 공진 주파수(보통 2~4kHz)에 맞춰 조정
...
| 부저 (선택) | 수동 피에조의 한쪽을 **GPIO10**(`SPK_PIN`)에, 반대쪽은 둘 중 하나로: ① **진짜 GND** (`SPK_GND_PIN -1`), ② SuperMini처럼 GND 핀이 부족하면 **가상 GND**(`SPK_GND_PIN`에 지정한 GPIO를 LOW로 고정, 기본 **21** — 20도 가능). GPIO21은 UART0 TX라 리셋 직후 ROM 부트 로그로 잠깐 지직거릴 수 있고, USB CDC On Boot를 끈 빌드와는 병행 불가. 피에조는 전류를 거의 먹지 않아 어느 쪽이든 GPIO 직결로 충분. 능동 부저(자체 발진)는 전류를 많이 먹어 트랜지스터 구동 필요. 부저 자체가 없으면 `SPK_PIN -1` |
...
- 스케치의 `U8G2_SH1106_128X64_NONAME_F_HW_I2C` 생성자는 **SSD1306 모듈과도 호환**됩니다(SH1106·SSD1306 두 패널에서 실사용 확인). U8g2의 SH1106 드라이버가 SSD1306용 초기화 시퀀스를 그대로 재사용하고(SSD1306 전용 차지펌프 명령 0x8D 포함 — SH1106은 모르는 명령이라 무시), 페이지 단위 전송의 SH1106용 +2 컬럼 오프셋은 그 초기화가 설정한 수평 자동증가 모드 덕에 SSD1306에서 무효화되기 때문입니다. 반대로 SSD1306 드라이버는 SH1106에 없는 명령(0x21/0x22)에 의존하므로 SH1106 패널에서는 화면이 밀리거나 깨집니다 — 이 두 종류를 오가는 환경이라면 SH1106 생성자가 안전한 공통분모입니다.
- 일부 패널(테스트한 1.3" SH1106 모듈)은 **화면 구동 중에 밝기 레지스터(0xD9/0xDB/0x81)를 다시 쓰면 검게 굳습니다**. 그래서 야간↔주간 전환 명령은 display off/on(0xAE/0xAF)으로 감싸 초기화 때와 같은 조건에서 보내고, 부팅 직후의 무의미한 "낮 값 재전송"은 생략합니다(0.96" 모듈은 어느 쪽이든 무해했음).
...
| `font_dseg14_r_11` | 요일(SUN~SAT)·PM 마커 — DSEG14 Classic Regular 11px, 15글자 |

한글은 U8g2 내장 `u8g2_font_gulim12_t_korean2`(KS X 1001 2,350자, 약 60KB)를 사용합니다. `korean1`(574자)에는 곡·망·백·복·윤·춘·충·칩·헌·휴 등이 없어 절기·공휴일 이름 표시가 불가능합니다.
...
| `BLE_DUTY_CYCLE` | 1 | 1이면 동기화 전후에만 BLE 라디오를 켬(평균 ~20mA), 0이면 상시 연결 유지(~50mA) |
| `BLE_SETTLE_MS` | 2초 | 동기화+ANCS 구독 확인 후 라디오를 끄기까지의 안정화 시간 |
| `BLE_LINGER_MS` | 60초 | 구독 확인이 안 됐을 때 동기화 후 라디오를 켜두는 상한(첫 페어링의 ANCS 허용 단계용) |
| `BLE_SYNC_TIMEOUT_MS` | 1분 | 동기화 없이 열려 있는 재동기화 창을 닫기까지의 시간(닫힐 때 아이콘 즉시 반전) |
...
| `BATTERY_DEBUG` | 0 | 1이면 좌상단에 아이콘 대신 측정 전압(mV) 표시 — `VBAT_DIV` 보정·배선 진단용 |
| `VBAT_LOW_PCT` | 10 | 이 잔량% 이하면 좌상단 아이콘이 깜빡임 |
...
| `WIFI_RETRY_MS` | 10초 | 등록된 AP가 안 보였을 때의 재스캔 주기(라디오 기동 직후의 빈 스캔 대비) |
...
| `DOUBLE_CLICK_MS` | 700 | 연속 클릭으로 인정할 클릭 간 최대 간격. 마지막 클릭 후 이 시간이 지나야 동작이 반영됨 |
...
| `SPK_PIN` | 10 | 피에조 부저의 톤 핀. **-1 = 부저 없음**(사운드 기능 전체 제거) |
| `SPK_GND_PIN` | 21 | 피에조 **반대쪽 다리**의 위치(`SPK_PIN`과 별개): LOW로 고정할 가상 GND GPIO(기본 21, 20도 가능), **-1 = 진짜 GND에 배선한 경우**(부저는 그대로 사용됨) |
| `SOUND_DEFAULT` | 1 | 첫 부팅 시 소리 켬(1)/뮤트(0). 이후 NVS에 저장된 값 사용 |
| `OVERRIDE_FACE` 표 참고 (`OVERRIDE_SOUND`) | -1 | 부팅 시 뮤트 상태를 소스에서 강제(0=뮤트, 1=켬) |
| `BOOT_BEEP` | 1 | 부팅 시 배선 확인용 "삐삑" 1회 |
| `HOURLY_CHIME` | 1 | 매 정시 "삐삑". 0으로 끔 |
| `CHIME_TONE_HZ` | 2500 | 시보 주파수 — 부저 공진에 맞춰 조정 |
| `CHIME_BEEP_MS` / `CHIME_GAP_MS` | 60 / 60 | 비프 길이와 간격 |
| `CHIME_FROM_HOUR` / `CHIME_TO_HOUR` | 7 / 22 | 시보가 울리는 시간대(포함 범위) |
