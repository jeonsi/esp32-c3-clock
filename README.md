...
- 하단 줄은 아이콘 바로 옆부터 왼쪽 정렬로 **음력 날짜 → 절기 → 기념일** 순서로 붙여서 배치합니다(음력–절기는 좁은 간격) — 밀착 배치로 남는 공간이 기념일 이름 몫입니다. `음`/`음 윤`(윤달)은 한글, 숫자는 DSEG. 공휴일 이름은 반전, 정월대보름·단오·칠석 같은 세시명절은 일반 글자. 한 줄에 다 들어가지 않으면 절기 → 음력 순으로 생략하고 이름은 항상 남깁니다.
...
- **좌상단에 배터리 아이콘**(채움 = 잔량%)과 잔량 % 숫자가 표시됩니다 — 날짜 줄이 중앙정렬이라 좌상단은 비어 있고, 우상단은 `SLEEP_DEBUG` 수치가 쓰므로 겹치지 않습니다. 잔량 10%(`VBAT_LOW_PCT`) 이하면 초마다 깜빡입니다. 전압→%는 리튬이온 OCV 곡선의 구간 보간(4.2V=100 … 3.5V≈8 … 3.0V=0)입니다.
- **저전압 보호**: EMA 전압이 3.0V(`VBAT_SHUTDOWN_MV`) 미만으로 3회 연속(30초) 측정되면 OLED를 끄고 deep sleep(~50µA)으로 들어가 셀을 과방전 구간에서 지킵니다. 1시간마다 깨어 전압을 재확인하므로 **충전기에 꽂아두면 스스로 복귀**합니다(리셋 버튼도 가능). 시각은 재부팅 후 BLE로 다시 받아오고 설정은 NVS에 남습니다.
- 배선 전에 플래시해도 안전합니다: 핀이 떠 있어 리튬이온답지 않은 값(2.5V 미만/4.5V 초과)이 읽히면 아이콘과 보호가 모두 비활성화됩니다. 배선 후에는 멀티미터로 배터리 전압을 실측해 표시값과 다르면 `VBAT_DIV`(기본 2.00)를 실측비로 보정하세요. `VBAT_ADC_PIN -1`이면 기능 전체가 빠집니다.
...
| `VBAT_DIV` | 2.00 | 분압비(Vbat/Vpin) — 멀티미터 실측으로 보정 |
| `VBAT_LOW_PCT` | 10 | 이 잔량% 이하면 좌상단 아이콘이 깜빡임 |
| `VBAT_SHUTDOWN_MV` | 3000 | 이 전압 미만 30초 지속 시 OLED 끄고 deep sleep(1시간마다 재확인). 0 = 사용 안 함 |
