...
ESP32-C3와 0.96" 128×64 OLED(SH1106/SSD1306, I2C)로 만든 NTP 시계입니다. [CYD Clock](https://github.com/jeonsi/cyd_clock_brightness)의 디지털/아날로그 화면을 128×64 흑백 화면에 옮긴 것으로, 시간 관리 방식과 음력·절기·공휴일 데이터(`korean_calendar.h`)를 그대로 가져왔습니다. 터치가 없는 대신 보드의 **BOOT 버튼** 하나로 화면 전환·12/24시간·야간 모드를 조작합니다.
...
```
   2026-08-30 (일)          ← 날짜(DSEG7 11px) + 한글 요일(굴림 12px). 일요일·공휴일은 요일 반전
...
### 시간 관리 (SNTP)
- ESP32 시스템 클럭을 SNTP로 디시플린. 30분마다 재동기화, `SNTP_SYNC_MODE_SMOOTH`로 시간이 점프하지 않고 서서히 보정
- NTP 서버: `kr.pool.ntp.org` → `pool.ntp.org` → `time.google.com`, 타임존 `KST-9`
- 비차단 부팅: Wi-Fi 연결/시간 동기화 진행 상황(경과 시간, 재시도 횟수)을 화면에 표시하고, 실패해도 30초마다 무한 재시도
...
| 1초 이상 | 야간 모드 ON/OFF | `야간 모드 켜짐` / `야간 모드 꺼짐` |

- 세 설정 모두 NVS(`Preferences`)에 저장되어 재부팅 후 유지. 설정이 바뀔 때만 기록
...
1. **보드**: ESP32 Arduino core 3.x (`esp32:esp32:esp32c3`)
2. **라이브러리** (Library Manager): `U8g2`
3. `secrets.h.example`을 `secrets.h`로 복사하고 **Wi-Fi SSID/비밀번호**를 입력 (`secrets.h`는 gitignore되어 커밋되지 않음)
4. 필요 시 스케치 상단의 튜닝 값(타임존, 야간 시간대, 12/24시간 기본값 등) 수정
5. 파일을 UTF-8로 저장(Arduino IDE 기본값) 후 업로드. 스케치는 기본 파티션에서 플래시의 약 83%를 사용합니다

소스 파일 구성: `esp32-c3-clock.ino`(동작 코드와 튜닝 값) · `clock_fonts.h`(DSEG 폰트, 생성물) · `korean_calendar.h`(음력·절기·공휴일 테이블) · `secrets.h`(Wi-Fi, gitignore) · `tools/`(폰트 생성 도구)
