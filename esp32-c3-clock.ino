...
#define DOUBLE_CLICK_MS      700                  // multi-click window: 1x face, 2x 12/24 h, 3x mute - clicks up
...
#endif
  // Measure the slept time with gettimeofday(): the system time is backed by
  // the RTC across light sleep, whereas esp_timer can miss the sleep
  // compensation and report ~0 for a real sleep.
  struct timeval tv0;
  gettimeofday(&tv0, NULL);
  esp_err_t err = esp_light_sleep_start();
  struct timeval tv1;
  gettimeofday(&tv1, NULL);
  uint32_t slept = (uint32_t)((tv1.tv_sec - tv0.tv_sec) * 1000000L + (tv1.tv_usec - tv0.tv_usec));
...
static void draw_clock(const struct tm & t) {
  ...
  // letter look low. Instead the glyph goes up one MORE row and gets a box
  // that hugs its ink with a 1 px pad whose BOTTOM edge lines up with the
  // digits' bottom row (verified pixel-for-pixel on the host emulator;
  // u8g2 puts a glyph's bottom row at y - yoff - 1).
  const int wy = DATE_NUM_Y - 2;
  if (day_info.red_day) {
    u8g2.drawBox(x - 1, wy - 11, wd_w + 2, 13);
    u8g2.setDrawColor(0);
    u8g2.drawUTF8(x, wy - 1, wd);
    u8g2.setDrawColor(1);
  } else {
